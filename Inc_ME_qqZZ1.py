...
def Calculate_ME(i):
    ...
    TarArea = scratchArea+"tmp_qqZZ_bkg_"+str(i)
    myfile = TFile('/home/net3/skrishna/minitrees/mc16e/mc16_13TeV.combined.Sherpa_222_NNPDF30NNLO_llll.root');

    #get the minitree from the TFile and loop over the entries
    mytree = myfile.Get('tree_incl_all')
    entries = mytree.GetEntriesFast()

    test = "mc16e_qqZZ_130_bkg" +str(i)+ ".root"
    f = TFile(test,'recreate')

    #creating a new directory in the scratch area and moving there
    if (os.path.isdir("tmp_qqZZ_bkg_"+str(i))==True):
        file_copy = "tmp_qqZZ_bkg_"+str(i)
        shutil.rmtree(file_copy)
       #shutil.rmtree("tmp_ggHZZ_"+str(i))
    os.mkdir("tmp_qqZZ_bkg_"+str(i))
    os.chdir("tmp_qqZZ_bkg_"+str(i))

    #defining i/o files for check_sa.f to read and write to prevent overwriting
    ps_input = TarArea + "/input_qqZZ_bkg_"+str(i)+".input"
    results = TarArea + "/output_qqZZ_bkg_"+str(i)+".dat"
...
# subprocess.call(["tar","-xzf","ggnoH_4lep_1j.tar.gz"])
    ...
    i_events = i*4000
    for jentry in range(i_events, i_events+4000):
    ...
    subprocess.call(["scp", minitree ,"abc-at13:/gpfs3/umass/HZZ/MG5_aMC_v3_0_0/test_copied_trees/qqZZ/mc16e/"])
