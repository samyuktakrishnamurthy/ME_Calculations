...
def Calculate_ME(i):
    ...
    TarArea = scratchArea+"tmp_data_run2_"+str(i)
    myfile = TFile('/home/net3/skrishna/minitrees/data/dataRun2All.root');

    #get the minitree from the TFile and loop over the entries
    mytree = myfile.Get('tree_incl_all')
    entries = mytree.GetEntriesFast()

    test = "data_run2" +str(i)+ ".root"
    f = TFile(test,'recreate')

    #creating a new directory in the scratch area and moving there
    if (os.path.isdir("tmp_data_run2_"+str(i))==True):
        file_copy = "tmp_data_run2_"+str(i)
        shutil.rmtree(file_copy)
      
    os.mkdir("tmp_data_run2_"+str(i))
    os.chdir("tmp_data_run2_"+str(i))

    #defining i/o files for check_sa.f to read and write to prevent overwriting
    ps_input = TarArea + "/input_data_run2"+str(i)+".input"
    results = TarArea + "/output_data_run2"+str(i)+".dat"
...
# subprocess.call(["tar","-xzf","ggnoH_4lep_1j.tar.gz"])
    ...
    subprocess.call(["scp", minitree ,"abc-at13:/gpfs3/umass/HZZ/MG5_aMC_v3_0_0/test_copied_trees/data/"])
